...
def main(args):
    ...
    common_policy_config = {
        ...
        # Stage-aware TRIR++ v7 training controls. These are ignored during
        # normal inference except that use_stage_pred must match checkpoints
        # trained with the auxiliary stage head.
        'use_trir': args.get('use_trir', False),
        'trir_aug_prob': args.get('trir_aug_prob', 0.0),
        'trir_view_prob': args.get('trir_view_prob', 1.0),
        'trir_aug_weight': args.get('trir_aug_weight', 0.0),
        'trir_cons_weight': args.get('trir_cons_weight', 0.0),
        'trir_feat_cons_weight': args.get('trir_feat_cons_weight', 0.0),
        'trir_brightness': args.get('trir_brightness', 0.0),
        'trir_contrast': args.get('trir_contrast', 0.0),
        'trir_gamma': args.get('trir_gamma', 0.0),
        'trir_saturation': args.get('trir_saturation', 0.0),
        'trir_blur_prob': args.get('trir_blur_prob', 0.0),
        'trir_shadow_prob': args.get('trir_shadow_prob', 0.0),
        'trir_shadow_strength': args.get('trir_shadow_strength', 0.0),
        'trir_erasing_prob': args.get('trir_erasing_prob', 0.0),
        'trir_noise_std': args.get('trir_noise_std', 0.0),
        'use_auto_stage_weight': args.get('use_auto_stage_weight', False),
        'stage_weight_max': args.get('stage_weight_max', 1.0),
        'stage_event_window': args.get('stage_event_window', 1),
        'stage_speed_power': args.get('stage_speed_power', 1.0),
        'stage_acc_power': args.get('stage_acc_power', 0.7),
        'stage_gripper_power': args.get('stage_gripper_power', 1.0),
        'stage_gripper_indices': args.get('stage_gripper_indices', '6,13'),
        'use_stage_pred': args.get('use_stage_pred', False),
        'stage_num': args.get('stage_num', 5),
        'stage_loss_weight': args.get('stage_loss_weight', 0.0),
        'stage_hidden_dim': args.get('stage_hidden_dim', 128),
    }
...
def eval_bc(config, ckpt_name, save_episode=True):
    ...
    try:
        loading_status = policy.load_state_dict(ckpt_state)
    except RuntimeError as exc:
        print('⚠️ Strict checkpoint loading failed. Retrying with strict=False.')
        print(exc)
        loading_status = policy.load_state_dict(ckpt_state, strict=False)
...
if __name__ == '__main__':
    ...
    # Stage-aware TRIR++ v7 training controls.
    parser.add_argument('--use_trir', action='store_true', help='Enable TRIR++ augmented visual consistency during training.')
    parser.add_argument('--trir_aug_prob', action='store', type=float, default=0.0)
    parser.add_argument('--trir_view_prob', action='store', type=float, default=1.0)
    parser.add_argument('--trir_aug_weight', action='store', type=float, default=0.0)
    parser.add_argument('--trir_cons_weight', action='store', type=float, default=0.0)
    parser.add_argument('--trir_feat_cons_weight', action='store', type=float, default=0.0)
    parser.add_argument('--trir_brightness', action='store', type=float, default=0.0)
    parser.add_argument('--trir_contrast', action='store', type=float, default=0.0)
    parser.add_argument('--trir_gamma', action='store', type=float, default=0.0)
    parser.add_argument('--trir_saturation', action='store', type=float, default=0.0)
    parser.add_argument('--trir_blur_prob', action='store', type=float, default=0.0)
    parser.add_argument('--trir_shadow_prob', action='store', type=float, default=0.0)
    parser.add_argument('--trir_shadow_strength', action='store', type=float, default=0.0)
    parser.add_argument('--trir_erasing_prob', action='store', type=float, default=0.0)
    parser.add_argument('--trir_noise_std', action='store', type=float, default=0.0)

    parser.add_argument('--use_auto_stage_weight', action='store_true', help='Automatically increase loss on inferred task-transition/contact phases.')
    parser.add_argument('--stage_weight_max', action='store', type=float, default=1.0)
    parser.add_argument('--stage_event_window', action='store', type=int, default=1)
    ...
    parser.add_argument('--stage_gripper_indices', action='store', type=str, default='6,13', help='Comma-separated action dims used as gripper/event cues, or none.')

    parser.add_argument('--use_stage_pred', action='store_true', help='Enable auxiliary stage prediction head and stage CE loss.')
    parser.add_argument('--stage_num', action='store', type=int, default=5)
    parser.add_argument('--stage_loss_weight', action='store', type=float, default=0.0)
