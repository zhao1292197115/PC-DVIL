...
class ACTPolicy(nn.Module):
    ...
    def __call__(self, qpos, image, actions=None, is_pad=None, return_outputs=False):
        ...
        if actions is not None: # training time
            ...
            l1 = (all_l1 * valid_mask).mean()
            loss_dict['l1'] = l1
            loss_dict['kl'] = total_kld[0]
            loss_dict['loss'] = loss_dict['l1'] + loss_dict['kl'] * self.kl_weight
            if return_outputs:
                # Used by TRIR training only. Do not add this tensor during normal
                # training/validation logging, otherwise history will keep large tensors.
