...
def main(args):
    ...
    common_policy_config = {
        ...
        'use_trir': args['use_trir'],
        'trir_aug_prob': args['trir_aug_prob'],
        'trir_view_prob': args['trir_view_prob'],
        'trir_aug_weight': args['trir_aug_weight'],
        'trir_cons_weight': args['trir_cons_weight'],
        'trir_noise_std': args['trir_noise_std'],
        'trir_erasing_prob': args['trir_erasing_prob'],
        'trir_max_erase_ratio': args['trir_max_erase_ratio'],
    }
...
def _sample_bool(prob):
    return bool(torch.rand((), device='cuda').item() < float(prob))


def apply_trir_visual_perturbation(
    image_data,
    aug_prob=0.5,
    view_prob=0.67,
    noise_std=0.015,
    erasing_prob=0.25,
    max_erase_ratio=0.25,
):
    """Apply task-irrelevant visual perturbations for TRIR training.

    Input shape: [B, K, C, H, W], value range [0, 1]. The augmentation changes
    illumination/background-like appearance only; qpos/actions are unchanged.
    It is intentionally lightweight and torch-only so it does not introduce an
    extra model dependency.
    """
    if float(aug_prob) <= 0 or not _sample_bool(aug_prob):
        return image_data

    x = image_data.clone()
    bsz, num_views, channels, height, width = x.shape
    changed_any = False

    for b in range(bsz):
        changed_this_sample = False
        for v in range(num_views):
            if not _sample_bool(view_prob):
                continue
            view = x[b, v]

            # Brightness / low-light / exposure perturbation.
            # Factor range is deliberately moderate to avoid destroying task geometry.
            brightness = torch.empty((), device=x.device).uniform_(0.55, 1.15)
            view = view * brightness

            # Contrast perturbation around per-view mean.
            contrast = torch.empty((), device=x.device).uniform_(0.75, 1.25)
            mean = view.mean(dim=(1, 2), keepdim=True)
            view = (view - mean) * contrast + mean

            # Mild color/channel gain; helps reduce dependence on fixed color balance.
            color_gain = torch.empty((channels, 1, 1), device=x.device).uniform_(0.85, 1.15)
            view = view * color_gain

            # Additive sensor-like noise.
            if float(noise_std) > 0:
                view = view + torch.randn_like(view) * float(noise_std)

            # Random erasing simulates local non-task occlusion / partial visual dropout.
            if float(erasing_prob) > 0 and _sample_bool(erasing_prob):
                ratio = float(torch.empty((), device=x.device).uniform_(0.08, max_erase_ratio).item())
                erase_h = max(1, int(height * ratio))
                erase_w = max(1, int(width * ratio))
                top = int(torch.randint(0, max(1, height - erase_h + 1), (), device=x.device).item())
                left = int(torch.randint(0, max(1, width - erase_w + 1), (), device=x.device).item())
                # Fill with the local/global view mean rather than black, so the model
                # does not overfit to artificial black squares.
                fill = view.mean(dim=(1, 2), keepdim=True)
                view[:, top:top + erase_h, left:left + erase_w] = fill

            x[b, v] = view.clamp(0.0, 1.0)
            changed_any = True
            changed_this_sample = True

        # Ensure at least one view is perturbed for samples selected by aug_prob.
        if not changed_this_sample and num_views > 0:
            v = int(torch.randint(0, num_views, (), device=x.device).item())
            brightness = torch.empty((), device=x.device).uniform_(0.55, 0.9)
            x[b, v] = (x[b, v] * brightness).clamp(0.0, 1.0)
            changed_any = True

    return x if changed_any else image_data


def forward_pass_trir(data, policy, config):
    """TRIR training forward pass.

    TRIR enforces action consistency between clean multi-view observations and
    task-irrelevant perturbed observations. It is training-only and does not add
    inference latency.
    """
    image_data, qpos_data, action_data, is_pad = data
    image_data = image_data.cuda(non_blocking=True)
    ...
    clean_dict = policy(qpos_data, image_data, action_data, is_pad, return_outputs=True)

    perturbed_image = apply_trir_visual_perturbation(
        image_data,
        aug_prob=config.get('trir_aug_prob', 0.5),
        view_prob=config.get('trir_view_prob', 0.67),
        noise_std=config.get('trir_noise_std', 0.015),
        erasing_prob=config.get('trir_erasing_prob', 0.25),
        max_erase_ratio=config.get('trir_max_erase_ratio', 0.25),
    )
    aug_dict = policy(qpos_data, perturbed_image, action_data, is_pad, return_outputs=True)

    valid_mask = clean_dict['valid_mask']
    # Consistency is applied on predicted action chunks only. Detaching neither side
    # lets the shared policy learn perturbation-invariant action outputs.
    cons = (torch.abs(clean_dict['a_hat'] - aug_dict['a_hat']) * valid_mask).mean()

    loss = (
        clean_dict['loss']
        + float(config.get('trir_aug_weight', 1.0)) * aug_dict['l1']
        + float(config.get('trir_cons_weight', 0.2)) * cons
    )
    ...
    out = {
        ...
        'l1': clean_dict['l1'],
        'kl': clean_dict['kl'],
        'trir_aug_l1': aug_dict['l1'],
        'trir_cons': cons,
    }
    return out


def detach_loss_dict(d):
    """Detach scalar logging entries and skip large auxiliary tensors."""
    skip_keys = {'a_hat', 'target_actions', 'valid_mask'}
    return {k: v.detach() for k, v in d.items() if k not in skip_keys}
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    print(
        ...
        f'use_trir={config.get("use_trir", False)}, '
        f'trir_aug_prob={config.get("trir_aug_prob", 0.0)}, '
        f'trir_cons_weight={config.get("trir_cons_weight", 0.0)}'
    )
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for _, data in enumerate(train_dataloader):
            with torch.autocast(device_type='cuda', dtype=amp_dtype, enabled=amp_enabled):
                if config.get('use_trir', False):
                    if policy_class != 'ACT':
                        raise NotImplementedError('TRIR is currently implemented for ACTPolicy only.')
                    forward_dict = forward_pass_trir(data, policy, config)
                else:
                    forward_dict = forward_pass(data, policy)
            ...
            optimizer.zero_grad(set_to_none=True)
            detached = detach_loss_dict(forward_dict)
...
def plot_history(train_history, validation_history, validation_epochs, num_epochs, ckpt_dir, seed):
    ...
    # TRIR adds train-only metrics such as trir_aug_l1 and trir_cons.
    # Validation summaries may not contain these keys, so plot validation only
    # when the same metric exists in validation_history.
    ...
    for key in keys:
        ...
        train_pairs = [(idx, summary[key]) for idx, summary in enumerate(train_history) if key in summary]
        if len(train_pairs) > 0:
            train_idx, train_raw = zip(*train_pairs)
            train_values = [_to_float_for_plot(v) for v in train_raw]
            if len(train_history) > 1:
                train_x_all = np.linspace(0, max(num_epochs - 1, 0), len(train_history))
                train_x = [train_x_all[i] for i in train_idx]
            else:
                train_x = [0]
        ...
        if len(validation_history) > 0:
            val_pairs = [
                (validation_epochs[idx], summary[key])
                for idx, summary in enumerate(validation_history)
                if key in summary
            ]
            if len(val_pairs) > 0:
                val_x, val_raw = zip(*val_pairs)
                val_values = [_to_float_for_plot(v) for v in val_raw]
                plt.plot(np.array(val_x), val_values, label='validation')
...
if __name__ == '__main__':
    ...
    # TRIR training-only controls. TRIR does not change inference graph or latency.
    parser.add_argument('--use_trir', action='store_true', help='Enable Task-Relevant Visual Invariance Regularization during training.')
    parser.add_argument('--trir_aug_prob', action='store', type=float, default=0.5, help='Probability of applying task-irrelevant visual perturbation to a training batch.')
    parser.add_argument('--trir_view_prob', action='store', type=float, default=0.67, help='Per-view probability of perturbation when TRIR is active.')
    parser.add_argument('--trir_aug_weight', action='store', type=float, default=1.0, help='Weight for perturbed-view BC L1 loss.')
    parser.add_argument('--trir_cons_weight', action='store', type=float, default=0.2, help='Weight for clean-vs-perturbed action consistency loss.')
    parser.add_argument('--trir_noise_std', action='store', type=float, default=0.015, help='Std of additive image noise for TRIR.')
    parser.add_argument('--trir_erasing_prob', action='store', type=float, default=0.25, help='Probability of random erasing per perturbed view.')
    parser.add_argument('--trir_max_erase_ratio', action='store', type=float, default=0.25, help='Maximum erase box side ratio for TRIR random erasing.')
