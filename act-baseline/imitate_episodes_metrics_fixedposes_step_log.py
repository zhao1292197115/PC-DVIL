...
def main(args):
    ...
    config = {
        ...
        'method_name': args.get('method_name', 'ACT-ResNet-3Cam'),
...
def save_step_logs(step_rows, step_log_dir, ckpt_name):
    """Save per-step rollout logs for reward/progress/jerk/latency curve plotting."""
    if not step_rows:
        return None
    os.makedirs(step_log_dir, exist_ok=True)
    metric_stem = ckpt_name.split('.')[0]
    csv_path = os.path.join(step_log_dir, f'step_log_{metric_stem}.csv')
    pd.DataFrame(step_rows).to_csv(csv_path, index=False)
...
def eval_bc(config, ckpt_name, save_episode=True):
    ...
    method_name = config.get('method_name', 'ACT-ResNet-3Cam')
...
if __name__ == '__main__':
    ...
    # Per-step logging controls for plotting reward/progress/jerk curves.
    parser.add_argument('--save_step_log', action='store_true', help='Save per-step reward/action/qpos/latency CSV during eval.')
    parser.add_argument('--step_log_dir', action='store', type=str, default=None, help='Directory for per-step CSV logs.')
    parser.add_argument('--method_name', action='store', type=str, default='ACT-ResNet-3Cam', help='Method name written into per-step logs.')
