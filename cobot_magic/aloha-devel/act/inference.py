...
def get_model_config(args):
    ...
    config = {
        ...
        'cache_interval': max(1, int(args.cache_interval)),
    }
    return config
...
def inference_process(args, config, ros_operator, policy, stats, t, pre_action):
    ...
    while True and not rospy.is_shutdown():
        ...
        # v5: inference-only safe mode. This keeps the rollout closed-loop but prevents
        # PyTorch from building a computation graph and uses AMP on CUDA to reduce VRAM.
        with torch.inference_mode():
            if torch.cuda.is_available():
                ...
                all_actions = policy(curr_image, curr_depth_image, qpos)
        if torch.is_tensor(all_actions):
...
def model_inference(args, config, ros_operator, save_episode=True):
    ...
    effective_lookahead_step = args.pos_lookahead_step
    if config.get('feature_cache', False):
        effective_lookahead_step = max(1, int(config.get('cache_interval', 2)))
        print(f"[Real Cache] enabled: reuse action chunk for {effective_lookahead_step} publish steps.")
                ...
                if config['policy_class'] == "ACT":
                    if t >= max_t:
                        ...
                        if inference_actions is not None:
                            ...
                            inference_actions = None
                            max_t = t + effective_lookahead_step
                            if config['temporal_agg']:
                    ...
                    if config['temporal_agg']:
                        ...
                        if effective_lookahead_step != 0:
                            raw_action = all_actions[:, t % effective_lookahead_step]
                        else:
...
def get_arguments():
    ...
    # real-robot DINOv2 backbone options; must match the training command.
